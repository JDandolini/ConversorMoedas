    ...
    function main() {
      let currency = { "BRL": "Real (R$)", "EUR": "Euro (£)", "USD": "Dólar Americano ($)", "CAD": "Dólar Canadense (C$)","GBP":"Libra Esterlina (£)","ARS":"Peso Argentino ($)","AUD":"Franco Suíço (Fr)","CNY":"Yuan Chinês (元)","ILS":"Novo Shekel Israelense (₪)","JPY":"Iene Japonês (¥)","UYU":"Peso Uruguaio ($)","PKR":"Rúpia Paquistanesa (Rs)","INR":"Rúpia Indiana(₹)","RUB":"Rublo Russo (₽)","KWD":"Dinar Kuwaitiano(د.ك)","SAR":"Riyal Saudita (﷼,)","VES":"Bolívar Venezuelano (Bs.F)","COP":"Peso Colombiano ($)","MXN":"Peso Mexicano (M$)","PYG":"Guarani Paraguaio (₲)","KYD":"Dólar das Ilhas Cayman (CI$)"
    };
      let options = [];
    ...
    async function calculate() {
      ...
      let rate = rates[to];
      tax_info.innerText = `1.00 ${getInfoSelect(label_from_currency)} = ${rate} ${getInfoSelect(label_to_currency)}`
